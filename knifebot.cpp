...
void Aimbot::knife() {
	...
	for (const auto& t : m_targets) {
		...
		if (g_extrapolation.HandleLagCompensation(t)) {
			...
			front->cache();
			math::VectorAngles(front->m_origin - g_cl.m_shoot_pos, a);
		...
		// we can history aim.
			...
			if (best->m_pred_origin.dist_to(g_cl.m_shoot_pos) <= max_dist) {
				...
				best->cache();
				math::VectorAngles(best->m_origin - g_cl.m_shoot_pos, a);
			...
			if (last && last != best) {
				...
				if (last->m_pred_origin.dist_to(g_cl.m_shoot_pos) <= max_dist) {
					...
					last->cache();
					math::VectorAngles(last->m_origin - g_cl.m_shoot_pos, a);
...
bool Aimbot::CanKnife(LagRecord* record, ang_t angle, bool& stab) {
	...
	if (health <= slash_dmg || record->m_pred_origin.dist_to(g_cl.m_shoot_pos) > 32.f)
		stab = false;
